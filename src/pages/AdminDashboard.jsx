...
const cards = [
  { label: "Customer accounts", value: "Manage access", icon: <FaUsers />, detail: "Review customer sign-in activity.", to: "/admin/login-activity", action: "View login activity" },
  { label: "Product catalogue", value: "Manage toys", icon: <FaBoxOpen />, detail: "Add, update, or review product listings.", to: "/shop", action: "Open catalogue" },
  { label: "Order activity", value: "Review orders", icon: <FaClipboardList />, detail: "Monitor customer purchases and fulfilment.", to: "/orders", action: "Open orders" },
];
...
function AdminDashboard() {
  ...
  return <main style={pageStyle}>
    ...
    <section style={gridStyle}>{cards.map((card) => <article key={card.label} style={cardStyle}>
      <span style={iconStyle}>{card.icon}</span><p style={labelStyle}>{card.label}</p><h2 style={{ margin: "0 0 10px", color: "#741b45" }}>{card.value}</h2><p style={{ margin: "0 0 18px", color: "#6b7280", lineHeight: 1.5 }}>{card.detail}</p><Link to={card.to} style={cardLinkStyle}>{card.action}</Link>
    </article>)}</section>
...
const cardLinkStyle = { display: "inline-block", color: "#a51d5d", fontWeight: 700, textDecoration: "none" };
